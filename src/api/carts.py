...
def set_item_quantity(cart_id: int, item_sku: str, cart_item: CartItem):
    ...
    with db.engine.begin() as connection:
        ...
        cost = price_of_potions[0]
        connection.execute(sqlalchemy.text(f"UPDATE cart SET quantity = {cart_item.quantity}, payment = payment + {cart_item.quantity * cost} WHERE cart_id = {cart_id};"))
...
def checkout(cart_id: int, cart_checkout: CartCheckout):
    ...
    with db.engine.begin() as connection:
        ...
        quantity = cart_checkout[0] 
        payment = cart_checkout[1]
        connection.execute(sqlalchemy.text(f"UPDATE global_inventory SET gold = gold + {payment}"))
